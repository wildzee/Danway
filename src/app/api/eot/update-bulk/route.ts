...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        return NextResponse.json({ success: true, count: updatedRecords.count, message: `Successfully updated ${updatedRecords.count} EOT records` });
    } catch (error) {
        return NextResponse.json({ success: false, error: "Failed to update EOT records" }, { status: 500 });
    }
}
